...
public final class CameraWrapper implements CameraAllCallback {
    ...
    private static final String TAG = "bbb";//CameraWrapper.class.getName();
...
//    private int mDefaultCameraId;
    ...
    private int mCurrentCameraId;  // Camera ID currently chosen
    ...
    private int interval = 5000;//循环对焦间隔
    /**
     * 闪关灯
     */
    ...
    private boolean mMeteringSupported;// 是否支持测光;

    /**
     * 快门声音
     */
    private boolean mSilenceOnTaken = true;
        ...
        if (mNumberOfCameras == 0) {
            ...
            for (int i = 0; i < mNumberOfCameras; i++) {
                ...
                if (cameraInfo.facing == Camera.CameraInfo.CAMERA_FACING_BACK) {
                    mCurrentCameraId = i;
//                Log.i(TAG, "ori:"+mCurrentOrientation);
...
//            Log.i(TAG, "pic width:"+picSizes.get(i).width+", height:"+picSizes.get(i).height);
//        }
        ...
        // 检测测光
        try {
            mMeteringSupported = mDefaultParameters.getMaxNumMeteringAreas() > 0;
        } catch (NumberFormatException e) {
            e.printStackTrace();
        } catch (Exception e) {
            e.printStackTrace();
    ...
    public void openCamera(int cameraId) {
        ...
        if (mCameraSurfaceView.getSurfaceHolder() != null) {
            Log.i(TAG, "openCamera--SurfaceHolder:" + mPreviewState.state);
            setPreviewDisplay(mCameraSurfaceView.getSurfaceHolder());

        }else if (mCameraSurfaceView.getSurfaceTexture() != null) {
            Log.i(TAG, "openCamera--SurfaceTexture:" + mPreviewState.state);
...
//        Log.i(TAG, "getOptimalPreviewSize width:"+width+", height:"+height);
        if (sizes == null) {
            return null;
        }
...
//            Log.i(TAG, "support size-> width:"+size.width+", size:"+size.height);
...
//        Log.i(TAG, "optimalSize--size width:" + optimalSize.width + ", height:" + optimalSize.height);
        if (optimalSize != null) {
            optimalSize.width = (int) (previewRatio * optimalSize.height);
        }

//        Log.i(TAG, "size width:" + optimalSize.width + ", height:" + optimalSize.height);
        return optimalSize;
    }
...
//            Log.i(TAG, "preview width:" + width + ", height:" + height);
            ...
            mCurrentParameters.setPreviewSize(width, height);
            mCurrentParameters.setPictureSize(width, height);
    ...
    public int patchPictureDegree() {
        int degree = mCurrentPictureOrientation + 90;
        Log.i(TAG, "patchPictureDegree degree:" + degree);
    ...
    public void patchPreviewAndPictureDegree() {
        ...
        CameraOrientationInfoArray[mCurrentCameraId][0] = degree;
        CameraOrientationInfoArray[mCurrentCameraId][2] = degree;
        Log.i(TAG, "patchPreviewAndPictureDegree degree:" + degree);
    ...
    public void setPreviewOrientation(int orientation) {
        if (mCamera != null) {
            ...
            Log.i(TAG, "setPreviewOrientation:" + mCurrentOrientation + ", isLandscape:" + isLandscape);
    ...
    public void startPreview() {
        Log.i(TAG, "startPreview--:" + mPreviewState.state);
        if (mCamera != null && mPreviewState == States.PREVIEW_IDLE) {
            ...
            autoLoopFocus(true);
//            Log.i(TAG, "startPreview-22-:" + mPreviewState.state);
        }
    }
...
//        Log.i(TAG, "releaseCamera");
    ...
    private synchronized void checkFocusStateAndTakePicture() {
        ...
        if (takePicture && (mFocusMode.equals(Camera.Parameters.FOCUS_MODE_INFINITY)
            ...
            Log.i(TAG, "checkFocusStateAndTakePicture:"+takePicture);
            takePicture = false;
            startTakePicture();
...
//                Toast.makeText(, "出现了异常", Toast.LENGTH_SHORT).show();
    ...
    public void onSurfaceViewCreate() {
        ...
        if (mPreviewState == States.PREVIEW_IDLE) {
            Log.i(TAG, "--onSurfaceViewCreate--resume--:" + mPreviewState.state);
            reopenCamera();

        } else {
            if (mCameraSurfaceView.getSurfaceHolder() != null) {
                Log.i(TAG, "onSurfaceViewCreate--SurfaceHolder:" + mPreviewState.state);
                setPreviewDisplay(mCameraSurfaceView.getSurfaceHolder());

            }else if (mCameraSurfaceView.getSurfaceTexture() != null) {
                Log.i(TAG, "onSurfaceViewCreate--SurfaceTexture:" + mPreviewState.state);
                setPreviewDisplay(mCameraSurfaceView.getSurfaceTexture());
    ...
    public void onSurfaceViewChange() {
        Log.i(TAG, "onSurfaceViewChange "+mPreviewState.state);
        if (mPreviewState == States.PREVIEW_IDLE) {
    ...
    public void onSurfaceViewDestory() {
        Log.i(TAG, "onSurfaceViewDestory");
        autoLoopFocus(false);
    ...
    public void setCameraZoom(int value) {
        ...
        isZooming = false;
        mCameraHandler.sendEmptyMessageDelayed(MSG_RESUME_LOOP_FOCUS, 5000);
    }
...
//        }
     ...
     * @param isMust
     */
    private void doFocus(boolean isMust) {
...
//                    if (focus) {
                    ...
                    //4秒之后自动清理对焦状态;
                    mCameraHandler.sendEmptyMessageDelayed(MSG_CANCLE_FOCUS, 4000);
                    mFocusStartTime = System.currentTimeMillis();
                    mCamera.autoFocus(this);
//                    }else{
//                        mFocusState = States.FOCUS_SUCCESS;
...
//                    }
    ...
    public void setFlashMode(int flashMode) {
        if (mFlashSupported && mFlashSupportedModeNums > 0) {
            ...
            Log.i(TAG, "mCurrentFlashMode:" + mCurrentFlashMode);
        }
    }
...
//        Log.i(TAG, "onAutoFocus:" + success + ", time:" + (System.currentTimeMillis() - mFocusStartTime));
    ...
    public void onError(int error, Camera camera) {
        ...
        if (error == Camera.CAMERA_ERROR_SERVER_DIED) {
            Log.i(TAG, "onError: CAMERA ERROR SERVER DIED ->" + error);
        }
        if (mCameraAllCallback != null) {
...
//            mCameraAllCallback.onPictureTaken(isFront() ? mirrorConver(data, false, true) : data, camera);
            }
        } else {
...
//        Log.i(TAG, "----------onScreenOrientationChanged--------");
        if (mCameraAllCallback != null) {
